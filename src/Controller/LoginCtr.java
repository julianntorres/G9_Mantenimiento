...
public void actionPerformed(ActionEvent e) {
    ...
    if (!empty){
        loginui.dispose();
        UserSession userSession = new UserSession(String.valueOf(id), id, this.type);
        if ("admins".equals(this.type)){
            admin_menu admin = new admin_menu(userSession);
            String sqlAudit = String.format("INSERT INTO `audit_admin`(`id_admin`, `description`) VALUES ('%d','%s')", administrador.getId_admin(),"Login de administrador");
            try {
        ...
        if("users".equals(this.type)){
            user_menu user = new user_menu(userSession);
            String sqlAudit = String.format("INSERT INTO `audit_user`(`id_user`, `description`) VALUES ('%d','%s')", usuario.getId_user(),"Login de usuario");
            try {
                new ConnectionPool().makeUpdate(sqlAudit);
            } catch (SQLException ex) {
                System.out.println(ex);
